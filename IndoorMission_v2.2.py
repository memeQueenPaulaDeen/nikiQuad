...
class Drone(): #Connects to dronekit vehicle
    ...
    def __init__(self, connection_string = None, vehicle = None):
        if not connection_string is None:
            print("Getting Params from vehicle...")
...
def AutopilotCommunication():
    ...
    waitForEmergecyLandSafeThread = threading.Thread(target=waitForEmergecyLandSafe)
    waitForEmergecyLandSafeThread.start()
    
    if missionIsIndoor == True:
        threading.Thread(target = AutopilotIndoorPositionWriteMgr).start()
        
        
        waitForEmergecyLandSafeThread.join()
        IMThread.start()
        print "starting Fence thread"
        fenceXreturn = 0
        fenceYreturn = 0
        Alt = 1
        fenceThread = threading.Thread(target=fence, args=(fenceXreturn,fenceYreturn,Alt))
        fenceThread.start()
        print "Fence Thread Started"
        emergencyLandThread = threading.Thread(target=emergencyLand)
        emergencyLandThread.start()
        print "starting emergency landing thread"
    ...
    if manualControlMode == [1]: # Remote Control Channel 6 is in Manual Mode
        ...
        if (localConnect == True):
            ...
            if AutopilotCommunication.gpsGlobalOriginSet == False:
                ...
                #AutopilotCommunication.drone.vehicle.parameters['GND_ALT_OFFSET']  = homeAlt
                #time.sleep(1)
                homeAlt = 0.3
...
def takeOff(aTargetAltitude,err):
    ...
    if not outOfBounds:
        ...
        print "FENCE: take off overruled. "


def fence(returnX,returnY,returnAlt):
    global outOfBounds
    ...
    while(xMin < x[0] and x[0] < xMax and yMin < y[0] and y[0] < yMax and -z[0] < zMax):
        time.sleep(.05) #quad is still in safe zone
    
    outOfBounds = True
    print "Quad Has Left Saftey Fence Returning to safe pos"
    
    
    speed = .5 #m/s
    AutopilotCommunication.drone.vehicle.airspeed = speed
    AutopilotCommunication.drone.vehicle.groundspeed = speed
    err = .5 #meters
    
    returnLoc = LocationGlobalRelative(getLat(returnX),getLong(returnY),returnAlt)
    AutopilotCommunication.drone.vehicle.simple_goto(returnLoc,speed)
    while waitForPos(x[0],y[0],z[0],returnX,returnY,returnAlt,err):
        time.sleep(.1)
        print "Returning to Safe postion"
    print "arrived at Safe position"
    
    print "Landing"
    AutopilotCommunication.drone.vehicle.mode = VehicleMode("LAND")
    print "Fence: Job Finished!"    
        
        

def setAttitude(pitchAngle,rollAngle,YawAngle,pitchRate,rollRate,yawRate,duration): # Input Values in Degrees
    
    
    def sendAttMsg(pitchAngle,rollAngle,YawAngle,pitchRate,rollRate,yawRate):
        q = quaternion.QuaternionBase([np.radians(float(rollAngle)), np.radians(float(pitchAngle)), np.radians(float(YawAngle))])
        print "setting attitude to pitch: " + str(pitchAngle) + " roll: " +str(rollAngle) + " yaw: " + str(YawAngle)
        print"PitchRate: "+str(pitchRate)+" RollRate: "+str(rollRate)+" YawRate: "+str(yawRate)
        msg = AutopilotCommunication.drone.vehicle.message_factory.set_attitude_target_encode(
        0, # figure time out!!!!!!!
        1,               #target system
        1,               #target component 
        0b00000000,      #type mask
        q,               #qauternian 
        np.radians(rollRate),               #body roll rate
        np.radians(pitchRate),               #body pitch rate
        np.radians(yawRate),               #body yaw rate
        0.5)               #thrust
        AutopilotCommunication.drone.vehicle.send_mavlink(msg)
        #AutopilotCommunication.drone.vehicle.flush()
    
    start = time.time()
    while time.time() - start < duration:
        if not outOfBounds:
            sendAttMsg(pitchAngle,rollAngle,YawAngle,pitchRate,rollRate,yawRate)
        else:
            print "FENCE: set attitude overruled. "
            break
        time.sleep(.1)
        
    if not outOfBounds:    
        sendAttMsg(0,0,YawAngle,pitchRate,rollRate,yawRate)
...
def testSetAttitude():
        ...
        setAttitude( pitchTestAng,0,165,1,1,1,1.5)
        setAttitude(-pitchTestAng,0,165,1,1,1,1.5)
        
        #goTo(2.63,0,Alt,AirSpeed,err)
        
        #setAttitude(0,rollTestAng,0,1)
        #goTo(0,0,Alt,AirSpeed,err)
        
        #setAttitude(0,0,yawTestAngle,1)
        #goTo(0,0,Alt,AirSpeed,err)
...
def systemIDMisson():
    print "System ID Mission Started.."
    try:             
        ...
        startXPitch = 1
        startYPitch = 0
        pitchHeading = 180 # Has to change to the actual South
        
        startXRoll = 0
        startYRoll = 1
        headingRoll = 90 # Has to change to the actual East
        
        startXYaw = 0
        startYYaw = 0
        startHeadingYaw =180 # Has to change to the actual South
        ...
        TsList = [2.5,2,1.5,1,.75,0.5,.4,.3,0.25,.2,0.125,.1,.06]
        manuverTimes = [y for x in tsList for y in [x,x]] #duplicate all values in above list
        
        #test pitch
        goTo(startXPitch,startYPitch,Alt,AirSpeed,err)
        setAttitude(pitchTestAng*direction,0,0)
        direction = -1
        for manTime in manuverTimes:
            
            setAttitude(pitchTestAng*direction,0,0)
            direction = direction * -1
            time.sleep(manTime) # Has to change to while (set Attitude)
        
        #test roll
        direction = -1
        for manTime in manuverTimes:
            if direction == -1:#return to starting Location
                goTo(startXRoll,startYRoll,Alt,AirSpeed,err)
                #need to add set heading angle 
                
            setAttitude(0,rollAng*direction,0)
            direction = direction * -1
            time.sleep(manTime)
        ...
        for manTime in manuverTimes:
            if direction == -1:#return to starting Location
                goTo(startXYaw,startYYaw,Alt,AirSpeed,err)
                #need to add set heading angle 
            
            setAttitude(0,0,yawAng*direction)
            direction = direction * -1
            time.sleep(manTime)
...
def goTo(targetX,targetY,targetAlt,AirSpeed,err):
    
    loc = LocationGlobalRelative(getLat(targetX),getLong(targetY),targetAlt)
    
    if not outOfBounds:
        AutopilotCommunication.drone.vehicle.simple_goto(loc,AirSpeed)
        while ( waitForPos(x[0],y[0],z[0],targetX,targetY,targetAlt,err) and not outOfBounds ) :
            time.sleep(.1)
            print "moving to target: X: " +str(targetX) + " Y: " +str(targetY) 
        print "arrived at target: X: " +str(targetX) + " Y: " +str(targetY) 
    else:
        print "FENCE: goTo overruled. "
...
def land():
    if not outOfBounds:
        ...
        print "FENCE: land overruled. "

def waitForEmergecyLandSafe(): # Wait for Manual switch to clear before running the mission
    print "enter"
    global manualControlMode
    print "manualControlMode = " +str(manualControlMode)
    while manualControlMode == [1]:  #while emergecy landing is active wait
        print "Autonomus Mission not allowed while emergecy land is state is active"
        time.sleep(1)
    print "exit"

def emergencyLand():
    global manualControlMode
    while manualControlMode == [0]:
        time.sleep(.1)
...
def testFence():
    ...
    takeOff(Alt,err)
    # goto A
    print "Position A should be out of bounds"
    goTo(Ax,Ay,Alt,AirSpeed,err)
...
def simple():
    ...
    goTo(Ax,Ay,Alt,AirSpeed,err)
    # goto B
    goTo(Bx,By,Alt,AirSpeed,err)
    # goto C
    goTo(Cx,Cy,Alt,AirSpeed,err)
    # goto D
    goTo(Dx,Dy,Alt,AirSpeed,err)
    # goto H
    goTo(Hx,Hy,Alt,AirSpeed,err)
    # land
    land()
...
def indoorMission():
    ...
    try: # write all mission code in this 'try' segment 
        ...
        #Arm the vehicle manually then set to guided mode to start the mission
        ...
        #testFence()#move out of bounds
        #superSimple()#take off and land
        simple()#follow course (move to corners of target)
        #testSetAttitude()
